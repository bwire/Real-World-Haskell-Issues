...
import System.FilePath(dropTrailingPathSeparator, splitFileName, (</>), isSearchPathSeparator)
...
listMatches dirName pattern = do
  ...
  handle ((\e  -> return []) :: SomeException -> IO [String]) $ do
    ...
    return (filter (`matchesGlob'` pattern) names') where
      matchesGlob' name pattern = matchesGlob name pattern $ isSearchPathSeparator ':'
